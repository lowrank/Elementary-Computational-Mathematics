...
The basic idea behind ``Richardson Extrapolation'' is to use polynomial interpolation with a sequence of nodes $h_j\to 0$. Suppose the function $A(h)$ admits the following asymptotic expansion:
\begin{equation}
...
\end{equation}
However, it is not guaranteed that such a process can constantly refine the approximation due to the potentially fast-growing constant in the $\cO$ notation.
...
\begin{example}
    ...
    \begin{enumerate}
        \item [$\circ$] Richardson Extrapolation. We take $1/h = 250, 500, 1000, 2000$ and calculate the Richardson extrapolation three times would result in almost machine accuracy.
        \item [$\circ$] Wynn's $\eps$ method. We take the sequence 
        $$s_k = \sum_{j=0}^k \frac{(-1)^j}{2 j + 1}$$
        as the truncated series at $z = 1$. With about 20 terms, we already reached the machine accuracy.
    \end{enumerate}
...
\end{equation}
This approximation offers a way to evaluate the derivative $f'(x)$ with the error term $\cO(h)$. Also, the above formula is \emph{exact} when $f$ is a polynomial of degree $1$. We say an approximation has ``degree $k$ accuracy'' if the approximation is \emph{exact} for any polynomial of degree $k$. 
...
\end{enumerate}
\begin{remark}
    The abscissa $\xi_j$, $j=0,\dots,n$ are in general distinct but possible to choose a single $\xi$ to simplify the representation through the intermediate value theorem. 
\end{remark}
