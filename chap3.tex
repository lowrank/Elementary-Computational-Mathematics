...
\begin{equation}
    s_n = \sum_{k=0}^n a_k, 
\end{equation}
under this case, the $A^k S$ is also called the Shanks transformation. When $k=1$, it corresponds to the standard Aitken extrapolation. The explicit form of $(A^k S)_n$ will be given in the next section.
...
\begin{algorithm}[!htb]
    ...
    \KwData{$s_0, s_1, \dots, s_n,\dots$}
    \KwResult{$\eps_{2l}^{(j)}$}
    \For{$j = 0, 1, 2,\dots$}{set 
    $\eps_{-1}^{(j)} = 0, \quad \eps_{0}^{(j)} = s_j.$ //    initialization; }
    \For{$j,k= 0, 1, 2,\dots$}{
      $$\eps_{k+1}^{(j)} = \eps_{k-1}^{(j+1)} + \frac{1}{\eps_{k}^{(j+1)} - \eps_{k}^{j}}.$$
    }
    \label{ALG: WYNN}
\end{algorithm}
...
Before we provide a connection between the table $\{ \eps_{2k}^{(n)} \}_{k\ge 0, n\ge 0}$ and Shanks transformation, let us first define the Hankel determinants for $k \ge 1$, 
\begin{equation}
...
\begin{lemma}[Sylvester determinantal identity]
...
\begin{proof}
 ...
 \right) = \det(A).
    \end{equation}
\end{proof}
\begin{lemma}[Schweinsian expansion]
\label{Lem: 4-SCH-EXP}
...
\begin{remark}
    ...
    \begin{equation}
    \begin{aligned}
    \cH_{k+2}(s_n) \cdot \cH_k(\Delta^2 s_n) - \cH_{k+1}(\Delta^2 s_n)\cdot \cH_{k+1}(s_{n+1}) = - \cH_{k+1}(\Delta s_{n})\cdot \cH_{k+1}(\Delta s_{n+1})
    \end{aligned}
    \end{equation}
    and 
    \begin{equation}
    ...
    \end{equation}
\end{remark}
% We can derive the following recursive relation for the Hankel determinants from Lemma~\ref{Lem: 4-SYL-DET-ID} directly.
%  \begin{equation}
%     \cH_{k+2}(s_n) \cdot \cH_{k}(s_{n+2}) = \cH_{k+1}(s_n) \cdot \cH_{k+1}(s_{n+2}) - \cH_{k+1}(s_{n+1}) ^2 .
% \end{equation}
\begin{theorem}
The $k$-th order of Shanks transformation equals
    \begin{equation}
        (A^k S)_n = \frac{\cH_{k+1}(s_n)}{\cH_{k}(\Delta^2 s_n)}.
    \end{equation}
\end{theorem}
\begin{proof}
