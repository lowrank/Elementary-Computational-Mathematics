...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
           P_{k+1} =  x P_k'(x) &= n(n-1)\cdots (n-k)(x-1)^{n-k-1} x^{k+1} + \\&\quad +   (x-1)^{n-k} \textcolor{red}{ n(n-1)\cdots (n-k+1) k x^k}  \\
           &\quad + (x-1)^{n-k} \textcolor{red}{(n-k+1)  F(x)}\\
           &\quad+(x-1)^{n-k}  \textcolor{red}{(x-1)F'(x)}.
        \end{aligned}
    \end{equation}
...
\end{theorem}
\begin{proof}
    The key idea is represent any polynomial $q$ of $\Pi_{n+m}$ by 
    \begin{equation}
        q(x) = \omega(x) s(x) + t(x)
    \end{equation}
...
For $w = 1$, the Legendre polynomial's roots are not at the end points, while sometimes the end points $\pm 1$ are useful to be included in the quadrature nodes, therefore we may want to generate a similar  Gauss quadrature with the end nodes as well. Following the same idea, in order to make $\pm 1$ as the roots of $\omega(x)$ but also keep $\omega$ concentrated on $p_{k}$ for large $k$, we can define
\begin{equation}
...
\end{theorem}
\begin{proof}
    Pick out all roots that are having odd multiplicity, say $z_0 < z_1 <\cdots < z_M$, then 
    \begin{equation}
...
\end{theorem}
\begin{proof}
   The polynomials $p_k$ satisfies the recursive formula 
   ...
   \begin{equation}
    ...
    \end{pmatrix}
   \end{equation}
   which means the matrix $A$ on left-hand-side has $(n+1)$ eigen-pairs $\{ x_l, (p_j(x_l))_{j=0}^n\} $. The tridiagonal matrix has a similar transform to a symmetric matrix, denoted by $S = D^{-1}A D$ with $D$ as a diagonal matrix, then $S$ has the same set of eigen-pairs, which means the transformed vectors 
   \begin{equation}
       D^{-1} \begin{pmatrix}
        p_0(x_l) \\ p_1(x_l) \\  p_2(x_l) \\ \vdots \\ p_{n-1}(x_l) \\ p_n(x_l)
       \end{pmatrix}
   ...
   \end{equation}
   where $v_{j} = (p_i(x_j))_{i=0}^n $. Take $f = p_k$ into the quadrature rule, 
   \begin{equation}
 ...
 = \delta_{0k}
   ...
   \end{equation}
\end{proof}
This result should be compared with Newton-Cotes formula, where the weights are not all positive if $n$ is large, hence Gauss quadrature has better numerical stability. 
At last, we briefly state the error estimate for the Gauss quadrature using the previous proved theorem~\ref{THM: ERROR QUAD RULE}, where $r = 2n+1$.
\begin{corollary}
...
\section{Gauss Quadrature on Unbounded Domain}
