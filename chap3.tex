...
\begin{equation}
    \cI_{\textrm{MC}, m} = \frac{\textrm{Vol}(D)}{N} \sum_{i=1}^m f(\bx_i) \sim \cI := \textrm{Vol}(D)\bbE_{\bx\sim \cU(D)} \left[f\right].
\end{equation}
As the sample points are independent, the law of large numbers (LLN) shows that
\begin{equation}
    \lim_{m\to \infty} \cI_{\textrm{MC}, m} = \cI.
\end{equation}
