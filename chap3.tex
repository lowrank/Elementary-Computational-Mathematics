...
Aitken extrapolation~\cite{aitken1927xxv} was introduced by Alexander Aitken and has been used for accelerating the rate of convergence of a sequence. 
...
\end{remark}
This extrapolation formula has been rediscovered by~\cite{schmidt1941xxxii} and~\cite{shanks1955non} independently. In~\cite{shanks1955non}, the sequence $S = \{s_n\}_{n\ge 0}$ is specified by the partial sum form,
\begin{equation}
    s_n = \sum_{k=0}^n a_k, 
\end{equation}
under this case, the Aitken extrapolation is also called the Shanks transformation. 
...
\begin{proof}
    ...
    \end{equation}
\end{proof}
\begin{remark} The Aitken extrapolation can be used to accelerate fixed-point iteration $x_{n+1} = f(x_n)$. Steffensen's method is based on such extrapolation.
\end{remark}
