...
\begin{remark}
    ...
    \end{equation}
    Moreover, using the Taylor expansion, $f[\underbrace{x_0,\dots, x_0}_{(k+1)\,\text{times}}] = \frac{1}{k!}f^{(k)}(x_0)$. However, in such cases, the divided differences cannot be computed if the derivative values are not provided. We will discuss this scenario later in the Hermite interpolation polynomial; see Section~\ref{SSec: 2-Her-Pol-Int}.
\end{remark}
\begin{remark}
...
\end{definition}
Using a simple induction, we can show the following property of the B-spline (see Exercise~\ref{Prb: 2-B-SPL-NON-SUP}).
...
\begin{proof}
    ...
    The equality can be proved by induction as well. For the induction step, by the recursive definition, we have
        ...
        \begin{aligned}
            ...
            &=  (x - \mu)  \sum_{j=r}^s\phi_{r,l}(\mu) B_{r, l}(x),\quad x\in (t_{r+l}, t_{s+1}). 
        \end{aligned}
        \end{equation}
    \end{proof}
    The B-splines of order $k$ naturally define a linear subspace $\cS_k$:
    \begin{equation}
        \cS_k = \text{span} \left\{ B_{jk}(x) \mid j\in \cJ\right\}.
    \end{equation}
    Then $\cS$ contains all polynomials of degree $\le k-1$. Furthermore, 
    
    
    is equivalent to piecewise polynomial function space $\Pi_{k, \bt, \bm}$ that 
    \begin{definition}
        $\Pi_{k, \bt, \bm}$ is the 
    \end{definition}
