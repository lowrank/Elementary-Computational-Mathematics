...
The definition of B-spline (short for basis spline) was initially proposed by~\cite{curry1947spline}. Let $\{t_j\}_{j\in \cJ}$ be a non-decreasing sequence, the index set $\cJ$ may be finite or infinite. The sequence is called ``knots''. The B-splines are defined in the following recurrence relation. 
...
and for $k > 1$, the B-spline of order $k$ satisfies
    $$B_{j,k}(x) = \frac{x - t_j}{t_{j+k-1} - t_j} B_{j, k-1}(x) + \frac{t_{j+1} - x}{t_{j+k} - t_{j+1}} B_{j+1, k-1}(x).$$
...
\label{Lem: 2-B-SPL-PAR-UNI}
   ...
   $$\sum_{j\in (\cJ + \{k\})\bigcap (\cJ -  \{k\})} B_{j, k}(x) = 1.$$
\end{lemma}
\begin{proof}
