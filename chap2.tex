...
\end{proof}
\begin{remark}
    This can be further extended to general continuous functions with \emph{modulus of continuity} $\omega(\frac{1}{n})\log n \to 0$ as $n\to\infty$, see the work of Dunham Jackson (1913). More related topics are discussed in Chapter~\ref{Ch: 4-App}.
\end{remark}
...
\end{equation}
Therefore, the interpolation error on $[a, b]$ is $\frac{1}{8} \|f''\|_{\infty} h^2$, where $h = \max|x_j - x_{j-1}|$.
...
\end{remark}
\subsection{Interpolation Error of Cubic Spline}
