...
\end{proof}
A precise characterization of the constant is conjectured as $D_{k,\infty} < c 2^{k}$; this is known as the ``De Boor's $2^k$ conjecture''~\cite{de1990exact}. Currently, it has been shown $D_{k,\infty} \le k 2^{k-1}$ by~\cite{scherer1999new} and $D_{k,\infty} \le \cO(\sqrt{k}\log k 2^{k})$ by~\cite{zhong2026improvement}, the conjecture still remains open.
