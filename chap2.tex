...
\label{Lem: 2-B-SPL-PAR-UNI}
   ...
   $$\sum_{j = r}^s B_{j, k}(x) = 1,\quad x\in (t_{r + k - 1}, t_{s+1}).$$
\end{lemma}
\begin{proof}
    Let us prove this by induction. For $k=1$, this is exactly the definition. Assume the conclusion for $k = l$, then for $x\in (t_{r + l}, t_{s+1})\subset (t_{r + l - 1}, t_{s+1})$, since $(t_{r + l}, t_{s+1})\bigcap \supp B_{r, l} = \emptyset$ and $(t_{r + l}, t_{s+1}) \bigcap \supp B_{s+1, l} = \emptyset$, we have 
    \begin{equation}
    \begin{aligned}
        \sum_{j=r}^s B_{j, l+1}(x) &=   \sum_{j=r}^s  \frac{x - t_j}{t_{j+l} - t_j} B_{j, l}(x) +   \sum_{j=r}^s\frac{t_{j+l+1} - x}{t_{j+l+1} - t_{j+1}} B_{j+1, l}(x) \\
        &=  \sum_{j=r}^s  \frac{x - t_j}{t_{j+l} - t_j} B_{j, l}(x) +  \sum_{j=r+1}^{s+1} \frac{t_{j+l} - x}{t_{j+l} - t_{j}} B_{j, l}(x) \\
        &= \sum_{j = r}^{s} B_{j, l}(x) - \frac{t_{r+l} - x}{t_{r+l} - t_r} B_{r, l}(x) +  \frac{t_{s+l +1} - x}{t_{s+l + 1} - t_{s+1}} B_{s+1, l}(x) \\
        &=1. 
    \end{aligned}
    \end{equation}
    ...
    \begin{equation}
    \label{EQ: MARSDEN}
        (x - \mu)^{k-1} = \sum_{j=r}^s \phi_{jk}(\mu) B_{jk}(x),\quad x\in (t_{r+k-1}, t_{s+1}), 
    \end{equation}
    ...
    The equality can be proved by induction as well. For the induction step, by the recursive definition, we have
        \begin{equation}\nonumber
        \begin{aligned}
            \sum_{j=r}^s \phi_{j,l+1}(\mu) B_{j, l+1}(x) &= \sum_{j=r}^s  \frac{x - t_j}{t_{j+l} - t_j} \phi_{j,l+1}(\mu) B_{j, l}(x) +   \sum_{j=r}^s\frac{t_{j+l+1} - x}{t_{j+l+1} - t_{j+1}} \phi_{j,l+1}(\mu) B_{j+1, l}(x)\\
            &=  \sum_{j=r}^s  \frac{x - t_j}{t_{j+l} - t_j}(t_{j+l}-\mu) \phi_{j,l}(\mu) B_{j, l}(x) \\&\quad +  \sum_{j=r+1}^{s+1} \frac{t_{j+l} - x}{t_{j+l} - t_{j}} (t_{j-1+l}-\mu)\phi_{j-1, l}(\mu)  B_{j, l}(x) \\
            &=  \sum_{j=r}^s  \frac{x - t_j}{t_{j+l} - t_j}(t_{j+l}-\mu) \phi_{j,l}(\mu) B_{j, l}(x) \\&\quad +  \sum_{j=r+1}^{s+1} \frac{t_{j+l} - x}{t_{j+l} - t_{j}} (t_{j}-\mu)\phi_{j, l}(\mu)  B_{j, l}(x)\\
            &=  (x - \mu)  \sum_{j=r}^s\phi_{j,l}(\mu) B_{j, l}(x),\quad x\in (t_{r+l}, t_{s+1})\subset (t_{r+l-1}, t_{s+1}). 
        \end{aligned}
        \end{equation}
    \end{proof}
    The B-splines of order $k$ naturally define a linear subspace $\cS_{k,\bt}$:
    \begin{equation}
        \cS_{k,\bt} = \text{span} \left\{ B_{jk}(x) \mid j\in \cJ\right\}.
    \end{equation}
    Then $\cS$ contains all polynomials of degree $\le k-1$. Indeed, differentiating~\eqref{EQ: MARSDEN} in $\mu$, one gets
    \begin{equation}
        \frac{(x - \mu)^{k - \nu}}{(k - \nu)!} = \sum_{j=r}^s\frac{(-1)^{\nu}\partial_{\mu}^{\nu} \phi_{jk}(\mu) B_{jk}(x)}{(k-1)!}, \quad x\in (t_{r+k-1}, t_{s+1}). 
    \end{equation}
    ...
    then for any $1\le \nu \le m_i$, we take $r = i - (k - \nu)$ and $\mu = t_i$, which leads to 
    \begin{equation}
        \frac{(x - t_i)^{k - \nu}}{(k - \nu)!} = \sum_{j= i - ( k -\nu)}^s \frac{(-1)^{\nu}\partial_{\mu}^{\nu} \phi_{jk}(t_i) B_{jk}(x)}{(k-1)!}, \quad x\in (t_{i + \nu-1}, t_{s+1}) = (t_i, t_{s+1}). 
    \end{equation}
    However, $\partial_{\mu}^{\nu} \phi_{jk}(t_i) = 0$ for $i-1 \ge j \ge i - (k-\nu)$, thus 
    \begin{equation}
         \frac{(x - t_i)^{k - \nu}}{(k - \nu)!} = \sum_{j= i}^s \frac{(-1)^{\nu}\partial_{\mu}^{\nu} \phi_{jk}(t_i) B_{jk}(x)}{(k-1)!}, \quad x\in (t_i, t_{s+1}). 
    \end{equation}
    We also notice $B_{jk}(x) = 0$ if $x\le t_i$ for all $j\ge i$, therefore 
    \begin{equation}
        \frac{(x - t_i)^{k - \nu}_{+}}{(k - \nu)!} = \sum_{j= i}^s \frac{(-1)^{\nu}\partial_{\mu}^{\nu} \phi_{jk}(t_i) B_{jk}(x)}{(k-1)!}, \quad x\le t_{s+1}.
    \end{equation}
    \end{proof}
    \begin{definition}
        Suppose $\bzeta = \{\zeta_i\}_{j=1}^{n+1}$ is an increasing sequence and denote $\bm = \{m_j\}_{j=2}^n$ be an nonnegative integer sequence that $m_j \le k$. Define $\Pi_{k, \bt, \bm}$ as the space of piecewise polynomials of degree at most $k-1$ with knots $\{t_i\}_{j=1}^n$ and satisfying the additional jump condition $\forall f\in \Pi_{k, \bzeta, \bm}$ 
        \begin{equation}
            \partial^{\nu-1} f(t_i^{+}) = \partial^{\nu-1} f(t_i^{-}),\quad 1 \le \nu \le m_i.
        \end{equation}
    \end{definition}
    \begin{theorem}[Curry-Schoenberg]
        ...
        \begin{equation}
            \underbrace{\zeta_1, \cdots, \zeta_1}_{\text{multiplicity } = k}, \underbrace{\zeta_2, \cdots, \zeta_2}_{\text{multiplicity } = k - m_2},\cdots, \underbrace{\zeta_{n}, \cdots, \zeta_n}_{\text{multiplicity } = k - m_n}, \underbrace{\zeta_{n+1}, \cdots, \zeta_{n+1}}_{\text{multiplicity } = k} 
        \end{equation}
        then on the interval $[t_{r+k-1}, t_{s+1}]$,
    ...
    \begin{proof}
        ...
        \begin{equation*}
            \begin{aligned}
                &(x - \mu)^{k-\nu}, \quad \nu = 1,\cdots, k; \\
                &(x - t_i)^{k - \nu}_{+},\quad i=2,\cdots, n,\quad 1\le \nu \le k - m_i.
            \end{aligned}
        \end{equation*}
       Therefore, $\Pi_{k,\bzeta, \bm}\subset \cS_{k, \bt}$. The dimension of $\cS_{k, \bt}$ is the number of B-splines $B_{jk}$, which is $|\bt| - k = n k - \sum_{i=2}^n m_i$ which matches to the total number of basis of $\Pi_{k,\bzeta, \bm}$. 
    \end{proof}
\subsection{B-spline Interpolation}
\label{SSec: 2-B-SPL-INT}
Let $n = \dim(\cS_{k, \bt})$ for the knots $\bt$. Since the B-splines $B_{jk}$ are linearly independent, then for the observation data $\{(x_i, y_i)\}_{i=1}^n$, the B-spline interpolation finds a spline $f(x) = \sum_{j=1}^n c_j B_{jk}$ that 
\begin{equation}
    \sum_{j=1}^n c_j B_{jk}(x_i) = y_i, i = 1,\cdots, n.
\end{equation}
Similar to polynomial interpolation, we carry out the criteria for existence and uniqueness first.
