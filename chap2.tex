...
\begin{proof}
     ...
     \end{equation*}
     which implies $\cK_n$ is the reproducing kernel on $H_n$. Then let $n\to\infty$, using the fact (see Exercise) that 
     \begin{equation}
...
Let $H$ be an RKHS on the non-empty set $\cX$ with the reproducing kernel $\cK(x, y)$. Suppose that the data $\{(x_j, y_j)\}_{j=1}^m$ are generated by $  y_j = f(x_j) = \aver{f, \cK(x_j)} $ for a unknown function $f\in H$. The interpolation finds a function $f^{\ast}$ such that 
\begin{equation}
    \aver{f^{\ast}, \cK(x_j)} = y_j,\quad j=1,\cdots, m.
\end{equation}
The choice for $f^{\ast}$ may not be unique. 

TODO: add the Theorem for minimal norm approximation.
...
\end{proof}
\begin{corollary}
    Suppose $H$ is an RKHS on $\bbR^d$ and let $G\subset \bbR^d$ be a subgroup of $\bbR^d$. If the reproducing kernel satisfies $\cK(x, y) = \cK(x - y)$ and  
    $$\sum_{g\in G, g\neq 0}|\cK(g)| < \cK(0),$$
    then $G$ is a UIS.
\end{corollary}
\begin{proof}
    The resulting matrix $A$ is an $M$ matrix. See Chapter~\ref{Ch: 8-Mat-Com}.
\end{proof}
