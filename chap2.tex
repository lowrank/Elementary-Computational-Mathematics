...
\end{theorem}
\begin{proof}
    We prove the RKHS is $H=\overline{\text{span}\{\cK(x, \cdot) \mid x\in \cX\}}$ equipped with the induced inner product $\aver{\cK(x, \cdot), \cK(y,\cdot)}_H = k(x, y)$. The verification is straightforward. For uniqueness, assume that $H'\supset H$ is another RKHS with kernel $\cK$ and inner product $\aver{\cdot, \cdot}_{H'}$. Then 
    \begin{equation}
...
Let $H$ be a RKHS with inner product $\aver{\cdot, \cdot}_H$. Suppose that the data $\{(\ell_j, z_j)\}_{j=1}^m$ are generated by $  z_j = \aver{\ell_j, f}_H $ such that $\ell_j\in H^{\ast}$ and $f\in H$ is a \emph{ unknown} function. The interpolation finds a function $f^{\ast}$ such that 
\begin{equation}
