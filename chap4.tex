...
\end{proof}
The following examples estimate the approximation error for Chebyshev polynomials and Legendre polynomials. The asymptotic expressions of Cauchy transforms are from~\cite{elliott1974asymptotic} for large $n$.  
\begin{example}
...
If $f\in C^{m-1}(I)$ with $f^{(m)}\in L^{\infty}(I)$ instead of being analytic, it is intuitive to conjecture that the coefficients $|a_n| = \cO(n^{-m})$ as an analog to Fourier approximation theory. Unlike the analytic case, extending the definition of $f$ to the Berstein ellipse is not applicable. 
...
\label{Lem: 4-Int-Ort-Pol}
    For Jacobi polynomials $p_n = P_{n}^{\alpha, \beta}$,  the constant $c_n = \frac{(-1)^{n}}{2^n n!}$ and
    \begin{equation}
...
\end{problem}
\begin{problem}
    Let Jacobi polynomials be defined as the following 
\end{problem}
